...
export function Explore() {
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0d] overflow-hidden">
      {/* Header with Search */}
      <div className="border-b border-[#808080]/30 bg-[#1a1a1a]/50 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="mb-4">
            <h1 className="text-4xl text-white mb-2 font-['DM_Sans'] font-bold">Explore</h1>
            <p className="text-[#C4C4C4]/70 font-['DM_Sans']">
              Discover inspiring restoration projects from the community
            </p>
          </div>
          <SearchBar />
        </div>
      ...
      >
        ...
        >
          {extendedProjects.map((project, index) => (
            <div key={`top-${index}`} className="flex-shrink-0 w-[380px] h-[480px]">
              <ProjectCard {...project} />
            </div>
          ))}
        </div>
        ...
        >
          {extendedProjects.map((project, index) => (
            <div key={`bottom-${index}`} className="flex-shrink-0 w-[380px] h-[480px]">
              <ProjectCard {...project} />
            </div>
          ))}
        </div>
